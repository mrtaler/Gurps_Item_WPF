...

{
    public class Category
    {
       public List<GurpsCategory> GurpsCategoryColleList { get; set; }
        ...
        {
            this.GurpsCategoryColleList=new List<GurpsCategory>();  
            ...
            if (xmlString.Contains(".skl"))
            {
                foreach (XElement skillElement in xdoc.Element("skill_list").Elements("skill").Elements("categories").Elements("category"))
                {
                    ...
                    int findIndex = this.GurpsCategoryColleList.BinarySearch(newGurpsCategory);
                    if (findIndex<0)
                    {
                        this.GurpsCategoryColleList.Insert(~findIndex, newGurpsCategory);
                    }
                }
